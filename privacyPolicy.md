...
- **我们的产品基于DCloud uni-app(5+ App/Wap2App)开发，应用运行期间可能需要收集您的设备唯一识别码（IMEI/android ID/DEVICE_ID/IDFA、SIM 卡 IMSI 信息、OAID）、设备型号、设备名称、SIM卡序列号、传感器、操作系统和应用程序版本等信息以提供对不同设备更好的适配功能，并通过应用启动数据及异常错误日志分析改进性能和用户体验，为用户提供更好的服务。**
...
1、为确保本应用处于关闭或后台运行状态下口正常接收到客户端推送的广播信息，本应用须使用(自启动)能力。将存在一定频率通过系统发送广播唤醒本应用自启动或关联启动行为，是因实现功能及服务所必要的。

2、当您打开外集卡路径引导app内容类推送消息，在征得您的明确同意后，会跳转外集卡路径引导app打开相关内容。在未征得您同意的情况下，则不会有关联启动。

3、当您打开外集卡路径引导app内部下载的文件后，会关联启动第三方APP
